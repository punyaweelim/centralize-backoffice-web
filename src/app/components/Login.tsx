...
export function Login() {
  ...
  const validateRole = (token: string): boolean => {
    try {
      ...
      if (isTokenExpired(token)) {
        setError('Token หมดอายุ กรุณาเข้าสู่ระบบใหม่อีกครั้ง', 'warning');
        return false;
      }
      
      // รองรับทั้ง role (string) และ roles (array)
      const userRole = decoded.role;
      const userRoles = decoded.roles || [];
      
      // ตรวจสอบว่ามีสิทธิ์เข้าใช้งานหรือไม่
      ...
      if (!hasPermission) {
        setError(
          'คุณไม่มีสิทธิ์เข้าใช้งาน Back Office\n' +
      ...
      setError('ไม่สามารถตรวจสอบสิทธิ์ได้ กรุณาลองใหม่อีกครั้ง', 'error');
      return false;
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try { 
      ...
      const hasPermission = validateRole(accessToken);
      ...
      setIsLoading(false);
      setProgress(0);
      apiClient.clearTokens(); // ล้าง token ถ้า login ไม่สำเร็จ
